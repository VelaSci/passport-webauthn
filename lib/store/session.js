...
SessionStore.prototype.verify = function (req, challenge, cb) {
  ...
  process.nextTick(function () {
    ...
    var issuedChallengeArrayBuffer = decode(info.challenge);
    var issuedChallenge = Buffer.from(new Uint8Array(issuedChallengeArrayBuffer))
    if (Buffer.compare(issuedChallenge, Buffer.from(new Uint8Array(challenge))) !== 0) {
      return cb(null, false, { message: 'Invalid challenge.' });
    }
    delete info.challenge;

    if (info.user && info.user.id) {
      info.user.id = base64url.decode(info.user.id);
    }
